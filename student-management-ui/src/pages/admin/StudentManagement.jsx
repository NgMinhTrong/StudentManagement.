...
const StudentManagement = () => {
    ...
    const fetchData = async () => {
        ...
        try {
            ...
            const studentUsers = userRes.data.filter(
                (u) => u.roles && u.roles.includes('Student')
            );
            setUsers(studentUsers);
        } catch (err) {
            console.error('Fetch Data Error:', err); // Log ra console để debug
            message.error(
                `Lỗi: ${err.response?.data?.message || 'Không thể tải dữ liệu từ hệ thống'}`
            );
        } finally {
            setLoading(false);
        }
    ...
    const handleDelete = async (id) => {
        const hide = message.loading('Đang xử lý xóa...', 0);
        try {
            ...
            hide();
            // Phân tích lỗi từ Backend trả về
            const errorMsg =
                err.response?.data?.message ||
                (err.response?.status === 500
                    ? 'Không thể xóa sinh viên này do có dữ liệu liên quan (điểm số, lớp học).'
                    : 'Lỗi hệ thống khi xóa sinh viên');
                ...
                <Form form={form} layout="vertical" style={{ marginTop: 20 }}>
                    ...
                    <Space direction="vertical" style={{ width: '100%' }}>
                        <Typography.Link style={{ fontSize: 12 }}>
                            + Tạo nhanh tài khoản mới (Tính năng sắp ra mắt)
                        </Typography.Link>
                    </Space>
                </Form>
            </Modal>
